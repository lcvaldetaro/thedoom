...
void P_CrossSpecialLine(line_t *line, int side, mobj_t *thing) {
    ...
    if (!demo_compatibility) // generalized types not recognized if old demo
    {
        ...
        if ((unsigned) line->special >= GenEnd) {
            ...
            if (((line->special & TriggerType) == WalkOnce) || ((line->special & TriggerType) == WalkMany)) { //jff 4/1/98 check for being a walk type before reporting door type
...
void P_ShootSpecialLine
    ...
    if (!demo_compatibility) {
        ...
        if ((unsigned) line->special >= GenEnd) {
            ...
            if (!thing->player)
                return;   // monsters disallowed from unlocking doors
            if (((line->special & TriggerType) == GunOnce) || ((line->special & TriggerType) == GunMany)) { //jff 4/1/98 check for being a gun type before reporting door type
                if (!P_CanUnlockGenDoor(line, thing->player))
                    return;
            } else
                return;
