...
#define PGMNAME "jni_doom.c"

// cacau this hack is to avoid an unsatisfied link on android 2.3 and earlier. Have no idea why. The library simply doesn't load without it
__aeabi_f2uiz() { ; }
...
void ii_printf(char *string, ...) {
    static char var_buf1[16384];
    va_list marker;
    va_start (marker, string);
    memset(var_buf1, 0, sizeof(var_buf1));
    vsprintf(var_buf1, string, marker);
#ifdef ANDROID
    __android_log_print(ANDROID_LOG_INFO, PGMNAME, "%s", var_buf1);
#endif
}

jint JNI_OnLoad(JavaVM *vm, void *reserved) {
    g_VM = vm;
    ii_printf("Library OnLoad called");
    return JNI_VERSION_1_4;
}

JNIEXPORT jint JNICALL Java_doom_jni_Natives_ChDir
        (JNIEnv *env, jclass class, jstring directory) {
    char *dir = (char *) (*env)->GetStringUTFChars(env, directory, 0);
    ii_printf("Library directory changed to '%s'", dir);
    chdir(dir);
    return 0;
}
/*
...
JNIEXPORT jint JNICALL Java_doom_jni_Natives_DoomMain
    ...
    ii_printf("Library DoomMain called");
    ...
    for (i = 0; i < clen; i++) {
        ...
        ii_printf("Main argv[%d]=%s", i, args[i]);
    ...
    {
        // TODO - change to API > 11 and use bellow
        // jclass tmp  = (*env)->FindClass(env, CB_CLASS);
        // jNativesCls = (jclass)env->NewGlobalRef(tmp);
        jNativesCls = (*env)->FindClass(env, CB_CLASS);

    }

    if (jNativesCls == 0) {
        ii_printf("Unable to find class: %s", CB_CLASS);
        return -1;
    }

    // Load doom.util.Natives.OnImageUpdate(char[])
    jSendImageMethod = (*env)->GetStaticMethodID(env, jNativesCls, CB_CLASS_IU_CB, CB_CLASS_IU_SIG);

    if (jSendImageMethod == 0) {
        ii_printf("Unable to find method OnImageUpdate(char[]): %s", CB_CLASS);
        return -1;
    }

    // Load OnStartSound(String name, int vol)
    jStartSoundMethod = (*env)->GetStaticMethodID(env, jNativesCls, CB_CLASS_SS_CB, CB_CLASS_SS_SIG);

    if (jStartSoundMethod == 0) {
        ii_printf("Unable to find method OnStartSound sig: %s ", CB_CLASS_SS_SIG);
        return -1;
    }
...
JNIEXPORT jint JNICALL Java_doom_jni_Natives_DoomkeyEvent
        (JNIEnv *env, jclass cls, jint type, jint key) {
    ii_printf("Library DoomkeyEvent called with type %d key %d", type, key);
...
*/
    ...
    jmethodID mid = (*env)->GetStaticMethodID(env, jNativesCls, CB_CLASS_IG_CB, CB_CLASS_IG_SIG);
...
void jni_send_pixels(int *data) {
    ...
    if (jSendImageMethod) {
        ...
        // Call Java method
        (*env)->CallStaticVoidMethod(env, jNativesCls, jSendImageMethod, jImage);
    }
...
static void jni_send_str(const char *text, int level) {
    ...
    // Call doom.doom.jni.Natives.OnMessage(String)
    if (!mSendStr) {
...
void jni_fatal_error(const char *text) {
    ...
    if (!g_VM) {
        ii_printf("JNI FATAL: No JNI Environment available. %s\n", text);
        exit(-1);
    }
    ...
    if (!env) {
        ii_printf("JNI FATAL: Unable to attach to cuur thread: %s.\n", text);
        exit(-1);
    }
    ...
    jmethodID mid = (*env)->GetStaticMethodID(env, jNativesCls, CB_CLASS_FATAL_CB, CB_CLASS_FATAL_SIG);

    if (mid) {
        (*env)->CallStaticVoidMethod(env, jNativesCls, mid, (*env)->NewStringUTF(env, text));
    } else {
        ii_printf("JNI FATAL: Unable to find method: %s, signature: %s\n", CB_CLASS_MSG_CB, CB_CLASS_MSG_SIG);
        exit(-1);
    }
...
void jni_start_sound(const char *name, int vol) {
     ...
     */
    JNIEnv *env;
    ii_printf("Library jni_start_sound begins name=%s, vol=%d", name, vol);

    if (!g_VM) {
        ii_printf("I_JNI: jni_start_sound No JNI Environment available.\n");
        return;
    }
    ...
    if (jStartSoundMethod == 0) {
        ii_printf("BUG: Invalid Doom JNI method method OnStartSound %s", CB_CLASS_SS_SIG);
        return;
    }
    ...
    ii_printf("Library jni_start_sound will call Java");

    // Call Java method

    jclass myclass;
    myclass = (*env)->FindClass(env, CB_CLASS);
    jNativesCls = (*env)->NewGlobalRef(env, myclass);

    (*env)->CallStaticVoidMethod(env, jNativesCls, jStartSoundMethod, jSound //(*env)->NewStringUTF(env, name)
            , (jint) vol);
...
void jni_start_music(const char *name, int loop) {
    ...
    jmethodID mid = (*env)->GetStaticMethodID(env, jNativesCls, CB_CLASS_SM_CB, CB_CLASS_SM_SIG);

    if (mid) {
        (*env)->CallStaticVoidMethod(env, jNativesCls, mid, (*env)->NewStringUTF(env, name), (jint) loop);
...
void jni_stop_music(const char *name) {
    ...
    jmethodID mid = (*env)->GetStaticMethodID(env, jNativesCls, CB_CLASS_STOPM_CB, CB_CLASS_STOPM_SIG);

    if (mid) {
        (*env)->CallStaticVoidMethod(env, jNativesCls, mid, (*env)->NewStringUTF(env, name)
        );
...
void jni_set_music_volume(int vol) {
    ...
    jmethodID mid = (*env)->GetStaticMethodID(env, jNativesCls, CB_CLASS_SETMV_CB, CB_CLASS_SETMV_SIG);

    if (mid) {
        (*env)->CallStaticVoidMethod(env, jNativesCls, mid, (jint) vol
        );
