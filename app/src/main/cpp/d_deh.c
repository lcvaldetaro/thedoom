...
static const deh_block deh_blocks[] = { // CPhipps - static const
        ...
        /* 13 */
                 {"[HELPER]",  deh_procHelperThing}, // helper thing substitution haleyjd 9/22/99
        /* 14 */
                 {"[SPRITES]", deh_procBexSprites}, // bex style sprites
        /* 15 */
                 {"[SOUNDS]",  deh_procBexSounds},   // bex style sounds
        /* 16 */
                 {"[MUSIC]",   deh_procBexMusic},     // bex style music
        /* 17 */
	...
	*/
    ...
    while (dehfgets(inbuffer, sizeof(inbuffer), filein)) {
        ...
        for (i = 0; i < DEH_BLOCKMAX; i++)
            if (!strncasecmp(inbuffer, deh_blocks[i].key, strlen(deh_blocks[i].key))) { // matches one
                if (fileout)
...
static void deh_procPointer(DEHFILE *fpin, FILE *fpout, char *line) // done
{
    ...
    while (!dehfeof(fpin) && *inbuffer && (*inbuffer != ' ')) {
        ...
        if (!strcasecmp(key, deh_state[4]))  // Codep frame (not set in Frame deh block)
        {
            ...
            if (fpout)
                fprintf(fpout, " - applied from codeptr[%lld] to states[%d]\n",
                        value, indexnum);
            ...
            fprintf(fpout, "Invalid frame pointer index for '%s' at %lld\n",
                    key, value);
    }
    return;
}
