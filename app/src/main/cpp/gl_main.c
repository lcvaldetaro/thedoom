...
void gld_Init(int width, int height) {
    ...
    } else if (!strcasecmp(gl_tex_filter_string, "GL_LINEAR_MIPMAP_NEAREST")) {
        use_mipmapping = true;
        gl_shared_texture_palette = false;
    ...
    } else if (!strcasecmp(gl_tex_filter_string, "GL_NEAREST_MIPMAP_LINEAR")) {
        use_mipmapping = true;
        gl_shared_texture_palette = false;
        lprintf(LO_INFO, "Using GL_NEAREST for normal textures.\n");
    ...
    } else if (!strcasecmp(gl_tex_filter_string, "GL_LINEAR_MIPMAP_LINEAR")) {
        use_mipmapping = true;
        gl_shared_texture_palette = false;
        lprintf(LO_INFO, "Using GL_LINEAR for normal textures.\n");
    ...
    } else if (!strcasecmp(gl_tex_filter_string, "GL_NEAREST")) {
    ...
    } else if (!strcasecmp(gl_tex_format_string, "GL_RGB5_A1")) {
        gl_tex_format = GL_RGB5_A1;
        lprintf(LO_INFO, "Using texture format GL_RGB5_A1.\n");
    } else if (!strcasecmp(gl_tex_format_string, "GL_RGBA4")) {
        gl_tex_format = GL_RGBA4;
        lprintf(LO_INFO, "Using texture format GL_RGBA4.\n");
    } else if (!strcasecmp(gl_tex_format_string, "GL_RGBA2")) {
...
void gld_DrawBackground(const char *name) {
    ...
    glTexCoord2f(fU1, fV2);
    glVertex2f((float) (0), (float) (0 + height));
    glTexCoord2f(fU2, fV1);
    glVertex2f((float) (0 + width), (float) (0));
    glTexCoord2f(fU2, fV2);
    glVertex2f((float) (0 + width), (float) (0 + height));
    glEnd();
}
...
void gld_EndDrawScene(void) {
    ...
    if (viewangleoffset <= 1024 << ANGLETOFINESHIFT ||
        viewangleoffset >= -1024 << ANGLETOFINESHIFT) { // don't draw on side views
        R_DrawPlayerSprites();
    }
        ...
        if (frontsector->floorpic == skyflatnum) {
            ...
            if (
                ...
                if ((texturetranslation[seg->sidedef->bottomtexture] != NO_TEXTURE)) {
                    ...
                    SKYTEXTURE(frontsector->sky, backsector->sky);
                    ADDWALL(&wall);
                } else if ((backsector->floorheight >= frontsector->ceilingheight) ||
                           (backsector->floorpic != skyflatnum)) {
                    wall.ytop = (float) backsector->floorheight / MAP_SCALE;
                    SKYTEXTURE(frontsector->sky, backsector->sky);
                    ADDWALL(&wall);
