...
public class DialogTool {
	...
	private static void loadSpinner(Context ctx, Spinner spinner, int resID,int idx) {
		ArrayAdapter<CharSequence> adapter = ArrayAdapter.createFromResource(ctx, resID, android.R.layout.simple_spinner_item);
	...
	public static void setLauncherDlgOptionsUI(final Context ctx, final View v,	boolean multiPlayer) {
		...
		/*
		 * // Load sound spinner loadSpinner(ctx, (Spinner)
		 * v.findViewById(R.id.s_sound), R.array.Sound, 0);
		 * 
		 * //Sizes spinner: portrait, landscape loadSpinner(ctx, (Spinner)
		 * v.findViewById(R.id.s_size), R.array.Size, 0);
		 */
	}
	 ...
	 * @param ctx
	 */
	static public void showNavMethodDialog(final Context ctx) { // , final
																// SensorManager
																// mSensorManager,
																// final
																// SensorListener
																// mSensor) {
		final String[] mListItems = new String[] {"Keyboard", "Touch Screen" }; // ,
																		// "Accelerometer"};

		AlertDialog dialog = new AlertDialog.Builder(ctx)
				.setIcon(R.drawable.icon).setTitle("Navigation Method")
				.setItems(mListItems, new DialogInterface.OnClickListener() {
					public void onClick(DialogInterface dialog, int which) {
						final View v = ((Activity) ctx)
								.findViewById(R.id.pan_ctls);
						final View v1 = ((Activity) ctx)
								.findViewById(R.id.other_ctls);
						...
						// if (!mSensorManager.registerListener(mSensor,
						// SensorManager.SENSOR_ACCELEROMETER)) {
						// MessageBox(ctx,
						// "Unable to register the accelerometer. Using keyboard by default.");
						// return;
						// }
						//
						// DoomClient.mNavMethod = eNavMethod.ACC; //
						// mAccelerometer = true;
						//
						// // Hide arrows
						// v.setVisibility(View.GONE);
						...
						//
						// Toast(ctx,
						// "Pitch controls Left/Right. Roll controls Up/Down");
						}
					}
				}).create();
		dialog.show();
	...
	static public AlertDialog createAlertDialog(Context ctx, String title,String message) {
		return new AlertDialog.Builder(ctx).setIcon(R.drawable.icon).setTitle(title).setMessage(message).create();
	}
	...
	public static void Toast(Handler handler, final Context ctx,
			final String text) {
